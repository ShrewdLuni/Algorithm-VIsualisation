...
namespace Algorithm_VIsualisation
{
    public static class SortingService
    {
        ...
        public static async Task BubbleSortAsync(int[] arr, int delay, IClientProxy clientProxy)
        {
        ...
        public static async Task QuickSortAsync(int[] arr, int delay, IClientProxy clientProxy)
        {
            await QuickSortHelperAsync(arr, 0, arr.Length - 1, delay, clientProxy);
            await clientProxy.SendAsync("SortComplete");
        }

        private static async Task QuickSortHelperAsync(int[] arr, int low, int high, int delay, IClientProxy clientProxy) 
        {
            if (high - low + 1 <= 1)
                return;
            ...
            await QuickSortHelperAsync(arr, low, left - 1, delay, clientProxy);
            await QuickSortHelperAsync(arr, left + 1, high, delay, clientProxy);
            await clientProxy.SendAsync("SortComplete");
        }

        public static async Task HeapSortAsync(int[] arr, int delay, IClientProxy clientProxy)
        {
        ...
        public static async Task RadixSortAsync(int[] arr, int delay, IClientProxy clientProxy)
        {
            int max = arr.Max();
            for (int exp = 1; max / exp > 0; exp *= 10)
            {
                await CountSortByDigitAsync(arr, exp, delay, clientProxy);
                await clientProxy.SendAsync("SortStep", arr, exp);
            }
            await clientProxy.SendAsync("SortComplete");
        }

        private static async Task CountSortByDigitAsync(int[] arr, int exp, int delay, IClientProxy clientProxy)
        {
            int n = arr.Length;
        ...
        public static async Task CocktailSortAsync(int[] arr, int delay, IClientProxy clientProxy)
        {
            bool swapped = true;
            int start = 0;
            int end = arr.Length;
        ...
        public static async Task SelectionSortAsync(int[] arr, int delay, IClientProxy clientProxy)
        {
            int minIndex = 0;
        ...
        public static async Task CountSortAsync(int[] arr, int delay, IClientProxy clientProxy)
        {
            int max = arr.Max();
